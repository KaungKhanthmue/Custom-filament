...
<div class="w-80 rounded bg-info px-6 pt-8 shadow-lg border border-gray-200">
    ...
    <div class="flex flex-col gap-3 border-b py-6 text-xs">
      ...
      <p class="flex justify-between">
        <span class="text-gray-400">Service Human:</span>
        <span>{{$serviceHuman}}</span>
      </p>
      <p class="flex justify-between">
    ...
    <div class="flex flex-col gap-3 pb-6 pt-2 text-xs">
      <table class="w-full text-left">
        ...
        @foreach($slectedProducts  as $product)
          <tr class="flex">
            <td class="flex-1 py-1">{{$product?->name}}</td>
            <td class="min-w-[44px]"></td>
            <td class="min-w-[44px]">{{$product?->price}}</td>
          </tr>
        @endforeach
        @else
        <tr class="flex">
